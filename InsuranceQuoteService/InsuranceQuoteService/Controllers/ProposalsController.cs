...
namespace InsuranceQuoteService.Presentation.Controllers
{
        ...
        ) : ControllerBase
    {
        ...
        public async Task<IActionResult> CreateProposal([FromBody, Required] CreateProposalRequestDto createProposalRequestDto)
        {
            ...
            var proposal = await _createProposalUseCase.ExecuteAsync(createProposalRequestDto);
        ...
        [ProducesResponseType((int)HttpStatusCode.InternalServerError)]
        public async Task<IActionResult> ListProposals([FromQuery] int pageIndex, [FromQuery] int pageSize)
        {
            Pagination pagination = new(pageIndex, pageSize);

            var proposals = await _listProposalsUseCase.ExecuteAsync(pagination);
        ...
        [ProducesResponseType((int)HttpStatusCode.InternalServerError)]
        public async Task<IActionResult> GetProposalById([FromRoute, Required] Guid id)
        {
            var proposal = await _getProposalByIdUseCase.ExecuteAsync(id);
        ...
        [ProducesResponseType((int)HttpStatusCode.InternalServerError)]
        public async Task<IActionResult> UpdateProposalStatus([FromRoute, Required] Guid id, [FromBody, Required] ProposalStatus newStatus)
        {
            await _updateProposalsStatusUseCase.ExecuteAsync(id, newStatus);
